...
shoppingCart.prototype.addItem = function (args) {
    ...
    if (document.getElementById('cr' + args.id) != null) {
        ...
        nameTdHtml += j6.template('<p class="qp"><a class="sub_btn" href="javascript:;" onclick="return cart.remove({goodsId},1)">-</a>'+
            '<input class="cart_q" value="{num}" type="text" onlur="return cart.setNum({goodsId});"/>'+
            '<a class="plus_btn" href="javascript:;" onclick="return cart.add({goodsId},1)">+</a></p>',
            {
                goodsId: args.id,
                num:args.num,
            });
