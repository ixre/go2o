// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        v7.34.1
// source: message/events_dto.proto

package proto

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// 商品分销设置
type EVItemAffiliateConfig struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// 分销比例
	Rate float64 `protobuf:"fixed64,2,opt,name=rate,proto3" json:"rate"`
	// 分销金额
	Amount int64 `protobuf:"varint,3,opt,name=amount,proto3" json:"amount"`
	// 是否按比例分销
	IsByRate      bool `protobuf:"varint,4,opt,name=isByRate,proto3" json:"isByRate"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *EVItemAffiliateConfig) Reset() {
	*x = EVItemAffiliateConfig{}
	mi := &file_message_events_dto_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EVItemAffiliateConfig) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EVItemAffiliateConfig) ProtoMessage() {}

func (x *EVItemAffiliateConfig) ProtoReflect() protoreflect.Message {
	mi := &file_message_events_dto_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EVItemAffiliateConfig.ProtoReflect.Descriptor instead.
func (*EVItemAffiliateConfig) Descriptor() ([]byte, []int) {
	return file_message_events_dto_proto_rawDescGZIP(), []int{0}
}

func (x *EVItemAffiliateConfig) GetRate() float64 {
	if x != nil {
		return x.Rate
	}
	return 0
}

func (x *EVItemAffiliateConfig) GetAmount() int64 {
	if x != nil {
		return x.Amount
	}
	return 0
}

func (x *EVItemAffiliateConfig) GetIsByRate() bool {
	if x != nil {
		return x.IsByRate
	}
	return false
}

// 订单分销商品
type EVOrderAffiliateItem struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// 商品编号
	ItemId int64 `protobuf:"varint,1,opt,name=itemId,proto3" json:"itemId"`
	// 商品SKU编号
	SkuId int64 `protobuf:"varint,2,opt,name=skuId,proto3" json:"skuId"`
	// 数量
	Quantity int32 `protobuf:"varint,3,opt,name=quantity,proto3" json:"quantity"`
	// 金额
	Amount int64 `protobuf:"varint,4,opt,name=amount,proto3" json:"amount"`
	// 最终金额, 可能会有优惠均摊抵扣的金额
	FinalAmount int64 `protobuf:"varint,5,opt,name=finalAmount,proto3" json:"finalAmount"`
	// 分销参数
	Params        []*EVItemAffiliateConfig `protobuf:"bytes,6,rep,name=Params,proto3" json:"Params"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *EVOrderAffiliateItem) Reset() {
	*x = EVOrderAffiliateItem{}
	mi := &file_message_events_dto_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EVOrderAffiliateItem) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EVOrderAffiliateItem) ProtoMessage() {}

func (x *EVOrderAffiliateItem) ProtoReflect() protoreflect.Message {
	mi := &file_message_events_dto_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EVOrderAffiliateItem.ProtoReflect.Descriptor instead.
func (*EVOrderAffiliateItem) Descriptor() ([]byte, []int) {
	return file_message_events_dto_proto_rawDescGZIP(), []int{1}
}

func (x *EVOrderAffiliateItem) GetItemId() int64 {
	if x != nil {
		return x.ItemId
	}
	return 0
}

func (x *EVOrderAffiliateItem) GetSkuId() int64 {
	if x != nil {
		return x.SkuId
	}
	return 0
}

func (x *EVOrderAffiliateItem) GetQuantity() int32 {
	if x != nil {
		return x.Quantity
	}
	return 0
}

func (x *EVOrderAffiliateItem) GetAmount() int64 {
	if x != nil {
		return x.Amount
	}
	return 0
}

func (x *EVOrderAffiliateItem) GetFinalAmount() int64 {
	if x != nil {
		return x.FinalAmount
	}
	return 0
}

func (x *EVOrderAffiliateItem) GetParams() []*EVItemAffiliateConfig {
	if x != nil {
		return x.Params
	}
	return nil
}

// 分销订单
type EVOrderAffiliateEventData struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// 订单号
	OrderNo string `protobuf:"bytes,1,opt,name=orderNo,proto3" json:"orderNo"`
	// 是否为子订单
	SubOrder bool `protobuf:"varint,2,opt,name=subOrder,proto3" json:"subOrder"`
	// 买家编号
	BuyerId int64 `protobuf:"varint,3,opt,name=buyerId,proto3" json:"buyerId"`
	// 订单金额
	OrderAmount int64 `protobuf:"varint,4,opt,name=orderAmount,proto3" json:"orderAmount"`
	// 分销商品
	AffiliateItems []*EVOrderAffiliateItem `protobuf:"bytes,5,rep,name=affiliateItems,proto3" json:"affiliateItems"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *EVOrderAffiliateEventData) Reset() {
	*x = EVOrderAffiliateEventData{}
	mi := &file_message_events_dto_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EVOrderAffiliateEventData) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EVOrderAffiliateEventData) ProtoMessage() {}

func (x *EVOrderAffiliateEventData) ProtoReflect() protoreflect.Message {
	mi := &file_message_events_dto_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EVOrderAffiliateEventData.ProtoReflect.Descriptor instead.
func (*EVOrderAffiliateEventData) Descriptor() ([]byte, []int) {
	return file_message_events_dto_proto_rawDescGZIP(), []int{2}
}

func (x *EVOrderAffiliateEventData) GetOrderNo() string {
	if x != nil {
		return x.OrderNo
	}
	return ""
}

func (x *EVOrderAffiliateEventData) GetSubOrder() bool {
	if x != nil {
		return x.SubOrder
	}
	return false
}

func (x *EVOrderAffiliateEventData) GetBuyerId() int64 {
	if x != nil {
		return x.BuyerId
	}
	return 0
}

func (x *EVOrderAffiliateEventData) GetOrderAmount() int64 {
	if x != nil {
		return x.OrderAmount
	}
	return 0
}

func (x *EVOrderAffiliateEventData) GetAffiliateItems() []*EVOrderAffiliateItem {
	if x != nil {
		return x.AffiliateItems
	}
	return nil
}

// 发送短信事件数据
type EVSendSmsEventData struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// 短信服务商
	Provider int32 `protobuf:"varint,1,opt,name=provider,proto3" json:"provider"`
	// 手机号
	Phone string `protobuf:"bytes,2,opt,name=phone,proto3" json:"phone"`
	// 短信内容
	Template string `protobuf:"bytes,3,opt,name=template,proto3" json:"template"`
	// 模板代码
	TemplateCode string `protobuf:"bytes,4,opt,name=templateCode,proto3" json:"templateCode"`
	// 短信模板
	SpTemplateId string `protobuf:"bytes,5,opt,name=spTemplateId,proto3" json:"spTemplateId"`
	// * 签名
	Signature string `protobuf:"bytes,6,opt,name=signature,proto3" json:"signature"`
	// 数据
	Data          []string `protobuf:"bytes,7,rep,name=data,proto3" json:"data"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *EVSendSmsEventData) Reset() {
	*x = EVSendSmsEventData{}
	mi := &file_message_events_dto_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EVSendSmsEventData) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EVSendSmsEventData) ProtoMessage() {}

func (x *EVSendSmsEventData) ProtoReflect() protoreflect.Message {
	mi := &file_message_events_dto_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EVSendSmsEventData.ProtoReflect.Descriptor instead.
func (*EVSendSmsEventData) Descriptor() ([]byte, []int) {
	return file_message_events_dto_proto_rawDescGZIP(), []int{3}
}

func (x *EVSendSmsEventData) GetProvider() int32 {
	if x != nil {
		return x.Provider
	}
	return 0
}

func (x *EVSendSmsEventData) GetPhone() string {
	if x != nil {
		return x.Phone
	}
	return ""
}

func (x *EVSendSmsEventData) GetTemplate() string {
	if x != nil {
		return x.Template
	}
	return ""
}

func (x *EVSendSmsEventData) GetTemplateCode() string {
	if x != nil {
		return x.TemplateCode
	}
	return ""
}

func (x *EVSendSmsEventData) GetSpTemplateId() string {
	if x != nil {
		return x.SpTemplateId
	}
	return ""
}

func (x *EVSendSmsEventData) GetSignature() string {
	if x != nil {
		return x.Signature
	}
	return ""
}

func (x *EVSendSmsEventData) GetData() []string {
	if x != nil {
		return x.Data
	}
	return nil
}

// 会员信息推送事件数据
type EVMemberPushEventData struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// * 编号
	MemberId int64 `protobuf:"varint,1,opt,name=memberId,proto3" json:"memberId"`
	// 是否新会员
	IsNewMember bool `protobuf:"varint,2,opt,name=isNewMember,proto3" json:"isNewMember"`
	// * 会员编码/邀请码
	UserCode string `protobuf:"bytes,3,opt,name=userCode,proto3" json:"userCode"`
	// * 会员标志
	UserFlag int32 `protobuf:"zigzag32,4,opt,name=userFlag,proto3" json:"userFlag"`
	// 角色标志,1: 用户, 2:商户雇员, 4:扩展1, 8:扩展2, 16:扩展3
	Role int32 `protobuf:"zigzag32,5,opt,name=role,proto3" json:"role"`
	// * 用户名
	Username string `protobuf:"bytes,6,opt,name=username,proto3" json:"username"`
	// * 国家代码
	CountryCode string `protobuf:"bytes,7,opt,name=countryCode,proto3" json:"countryCode"`
	// * 等级
	Level int32 `protobuf:"zigzag32,8,opt,name=level,proto3" json:"level"`
	// * 昵称
	Nickname string `protobuf:"bytes,9,opt,name=nickname,proto3" json:"nickname"`
	// * 会员头像
	ProfilePhoto string `protobuf:"bytes,10,opt,name=profilePhoto,proto3" json:"profilePhoto"`
	// * 手机号
	Phone string `protobuf:"bytes,12,opt,name=phone,proto3" json:"phone"`
	// * 电子邮箱
	Email string `protobuf:"bytes,13,opt,name=email,proto3" json:"email"`
	// * 注册来源
	RegFrom string `protobuf:"bytes,14,opt,name=regFrom,proto3" json:"regFrom"`
	// * 真实姓名
	RealName string `protobuf:"bytes,15,opt,name=realName,proto3" json:"realName"`
	// 邀请人
	InviterId     int64 `protobuf:"varint,16,opt,name=inviterId,proto3" json:"inviterId"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *EVMemberPushEventData) Reset() {
	*x = EVMemberPushEventData{}
	mi := &file_message_events_dto_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EVMemberPushEventData) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EVMemberPushEventData) ProtoMessage() {}

func (x *EVMemberPushEventData) ProtoReflect() protoreflect.Message {
	mi := &file_message_events_dto_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EVMemberPushEventData.ProtoReflect.Descriptor instead.
func (*EVMemberPushEventData) Descriptor() ([]byte, []int) {
	return file_message_events_dto_proto_rawDescGZIP(), []int{4}
}

func (x *EVMemberPushEventData) GetMemberId() int64 {
	if x != nil {
		return x.MemberId
	}
	return 0
}

func (x *EVMemberPushEventData) GetIsNewMember() bool {
	if x != nil {
		return x.IsNewMember
	}
	return false
}

func (x *EVMemberPushEventData) GetUserCode() string {
	if x != nil {
		return x.UserCode
	}
	return ""
}

func (x *EVMemberPushEventData) GetUserFlag() int32 {
	if x != nil {
		return x.UserFlag
	}
	return 0
}

func (x *EVMemberPushEventData) GetRole() int32 {
	if x != nil {
		return x.Role
	}
	return 0
}

func (x *EVMemberPushEventData) GetUsername() string {
	if x != nil {
		return x.Username
	}
	return ""
}

func (x *EVMemberPushEventData) GetCountryCode() string {
	if x != nil {
		return x.CountryCode
	}
	return ""
}

func (x *EVMemberPushEventData) GetLevel() int32 {
	if x != nil {
		return x.Level
	}
	return 0
}

func (x *EVMemberPushEventData) GetNickname() string {
	if x != nil {
		return x.Nickname
	}
	return ""
}

func (x *EVMemberPushEventData) GetProfilePhoto() string {
	if x != nil {
		return x.ProfilePhoto
	}
	return ""
}

func (x *EVMemberPushEventData) GetPhone() string {
	if x != nil {
		return x.Phone
	}
	return ""
}

func (x *EVMemberPushEventData) GetEmail() string {
	if x != nil {
		return x.Email
	}
	return ""
}

func (x *EVMemberPushEventData) GetRegFrom() string {
	if x != nil {
		return x.RegFrom
	}
	return ""
}

func (x *EVMemberPushEventData) GetRealName() string {
	if x != nil {
		return x.RealName
	}
	return ""
}

func (x *EVMemberPushEventData) GetInviterId() int64 {
	if x != nil {
		return x.InviterId
	}
	return 0
}

// 注册表推送事件数据
type EVRegistryPushEventData struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// 配置键
	Key string `protobuf:"bytes,1,opt,name=key,proto3" json:"key"`
	// 配置值
	Value string `protobuf:"bytes,2,opt,name=value,proto3" json:"value"`
	// 是否为用户创建的配置
	IsUsers       bool `protobuf:"varint,3,opt,name=isUsers,proto3" json:"isUsers"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *EVRegistryPushEventData) Reset() {
	*x = EVRegistryPushEventData{}
	mi := &file_message_events_dto_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EVRegistryPushEventData) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EVRegistryPushEventData) ProtoMessage() {}

func (x *EVRegistryPushEventData) ProtoReflect() protoreflect.Message {
	mi := &file_message_events_dto_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EVRegistryPushEventData.ProtoReflect.Descriptor instead.
func (*EVRegistryPushEventData) Descriptor() ([]byte, []int) {
	return file_message_events_dto_proto_rawDescGZIP(), []int{5}
}

func (x *EVRegistryPushEventData) GetKey() string {
	if x != nil {
		return x.Key
	}
	return ""
}

func (x *EVRegistryPushEventData) GetValue() string {
	if x != nil {
		return x.Value
	}
	return ""
}

func (x *EVRegistryPushEventData) GetIsUsers() bool {
	if x != nil {
		return x.IsUsers
	}
	return false
}

// 订单推送事件
type EVSubOrderPushEventData struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// 订单号
	OrderNo string `protobuf:"bytes,1,opt,name=orderNo,proto3" json:"orderNo"`
	// 订单金额
	OrderAmount int64 `protobuf:"varint,2,opt,name=orderAmount,proto3" json:"orderAmount"`
	// 收货人
	ConsigneeName string `protobuf:"bytes,3,opt,name=consigneeName,proto3" json:"consigneeName"`
	// 收货电话
	ConsigneePhone string `protobuf:"bytes,4,opt,name=consigneePhone,proto3" json:"consigneePhone"`
	// 收货地址
	ConsigneeAddress string `protobuf:"bytes,5,opt,name=consigneeAddress,proto3" json:"consigneeAddress"`
	// 状态
	OrderState    int32 `protobuf:"varint,6,opt,name=orderState,proto3" json:"orderState"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *EVSubOrderPushEventData) Reset() {
	*x = EVSubOrderPushEventData{}
	mi := &file_message_events_dto_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EVSubOrderPushEventData) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EVSubOrderPushEventData) ProtoMessage() {}

func (x *EVSubOrderPushEventData) ProtoReflect() protoreflect.Message {
	mi := &file_message_events_dto_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EVSubOrderPushEventData.ProtoReflect.Descriptor instead.
func (*EVSubOrderPushEventData) Descriptor() ([]byte, []int) {
	return file_message_events_dto_proto_rawDescGZIP(), []int{6}
}

func (x *EVSubOrderPushEventData) GetOrderNo() string {
	if x != nil {
		return x.OrderNo
	}
	return ""
}

func (x *EVSubOrderPushEventData) GetOrderAmount() int64 {
	if x != nil {
		return x.OrderAmount
	}
	return 0
}

func (x *EVSubOrderPushEventData) GetConsigneeName() string {
	if x != nil {
		return x.ConsigneeName
	}
	return ""
}

func (x *EVSubOrderPushEventData) GetConsigneePhone() string {
	if x != nil {
		return x.ConsigneePhone
	}
	return ""
}

func (x *EVSubOrderPushEventData) GetConsigneeAddress() string {
	if x != nil {
		return x.ConsigneeAddress
	}
	return ""
}

func (x *EVSubOrderPushEventData) GetOrderState() int32 {
	if x != nil {
		return x.OrderState
	}
	return 0
}

// * 账户
type EVMemberAccountEventData struct {
	state    protoimpl.MessageState `protogen:"open.v1"`
	MemberId int64                  `protobuf:"varint,1,opt,name=memberId,proto3" json:"memberId"`
	// 积分
	Integral int64 `protobuf:"varint,2,opt,name=integral,proto3" json:"integral"`
	// 商城余额
	Balance int64 `protobuf:"varint,3,opt,name=balance,proto3" json:"balance"`
	// 钱包代码
	WalletCode string `protobuf:"bytes,4,opt,name=walletCode,proto3" json:"walletCode"`
	// 钱包余额
	WalletBalance int64 `protobuf:"varint,5,opt,name=walletBalance,proto3" json:"walletBalance"`
	// 浮动账户余额
	FlowBalance int64 `protobuf:"varint,6,opt,name=flowBalance,proto3" json:"flowBalance"`
	// 增利账户余额
	GrowBalance int64 `protobuf:"varint,7,opt,name=growBalance,proto3" json:"growBalance"`
	// 总消费金额
	TotalExpense int64 `protobuf:"varint,8,opt,name=totalExpense,proto3" json:"totalExpense"`
	// 总充值金额
	TotalCharge int64 `protobuf:"varint,9,opt,name=totalCharge,proto3" json:"totalCharge"`
	// 更新时间
	UpdateTime    int64 `protobuf:"varint,20,opt,name=updateTime,proto3" json:"updateTime"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *EVMemberAccountEventData) Reset() {
	*x = EVMemberAccountEventData{}
	mi := &file_message_events_dto_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EVMemberAccountEventData) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EVMemberAccountEventData) ProtoMessage() {}

func (x *EVMemberAccountEventData) ProtoReflect() protoreflect.Message {
	mi := &file_message_events_dto_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EVMemberAccountEventData.ProtoReflect.Descriptor instead.
func (*EVMemberAccountEventData) Descriptor() ([]byte, []int) {
	return file_message_events_dto_proto_rawDescGZIP(), []int{7}
}

func (x *EVMemberAccountEventData) GetMemberId() int64 {
	if x != nil {
		return x.MemberId
	}
	return 0
}

func (x *EVMemberAccountEventData) GetIntegral() int64 {
	if x != nil {
		return x.Integral
	}
	return 0
}

func (x *EVMemberAccountEventData) GetBalance() int64 {
	if x != nil {
		return x.Balance
	}
	return 0
}

func (x *EVMemberAccountEventData) GetWalletCode() string {
	if x != nil {
		return x.WalletCode
	}
	return ""
}

func (x *EVMemberAccountEventData) GetWalletBalance() int64 {
	if x != nil {
		return x.WalletBalance
	}
	return 0
}

func (x *EVMemberAccountEventData) GetFlowBalance() int64 {
	if x != nil {
		return x.FlowBalance
	}
	return 0
}

func (x *EVMemberAccountEventData) GetGrowBalance() int64 {
	if x != nil {
		return x.GrowBalance
	}
	return 0
}

func (x *EVMemberAccountEventData) GetTotalExpense() int64 {
	if x != nil {
		return x.TotalExpense
	}
	return 0
}

func (x *EVMemberAccountEventData) GetTotalCharge() int64 {
	if x != nil {
		return x.TotalCharge
	}
	return 0
}

func (x *EVMemberAccountEventData) GetUpdateTime() int64 {
	if x != nil {
		return x.UpdateTime
	}
	return 0
}

// 提现申请推送事件
type EVMemberWithdrawalPushEventData struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// 会员编号
	MemberId int64 `protobuf:"varint,1,opt,name=memberId,proto3" json:"memberId"`
	// 流水号Id
	RequestId int64 `protobuf:"varint,2,opt,name=requestId,proto3" json:"requestId"`
	// 提现金额
	Amount int64 `protobuf:"varint,3,opt,name=amount,proto3" json:"amount"`
	// 手续费
	TransactionFee int64 `protobuf:"varint,4,opt,name=transactionFee,proto3" json:"transactionFee"`
	// 是否为已审核通过的事件
	IsReviewEvent bool `protobuf:"varint,5,opt,name=isReviewEvent,proto3" json:"isReviewEvent"`
	// 是否审核通过
	ReviewResult  bool `protobuf:"varint,6,opt,name=reviewResult,proto3" json:"reviewResult"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *EVMemberWithdrawalPushEventData) Reset() {
	*x = EVMemberWithdrawalPushEventData{}
	mi := &file_message_events_dto_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EVMemberWithdrawalPushEventData) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EVMemberWithdrawalPushEventData) ProtoMessage() {}

func (x *EVMemberWithdrawalPushEventData) ProtoReflect() protoreflect.Message {
	mi := &file_message_events_dto_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EVMemberWithdrawalPushEventData.ProtoReflect.Descriptor instead.
func (*EVMemberWithdrawalPushEventData) Descriptor() ([]byte, []int) {
	return file_message_events_dto_proto_rawDescGZIP(), []int{8}
}

func (x *EVMemberWithdrawalPushEventData) GetMemberId() int64 {
	if x != nil {
		return x.MemberId
	}
	return 0
}

func (x *EVMemberWithdrawalPushEventData) GetRequestId() int64 {
	if x != nil {
		return x.RequestId
	}
	return 0
}

func (x *EVMemberWithdrawalPushEventData) GetAmount() int64 {
	if x != nil {
		return x.Amount
	}
	return 0
}

func (x *EVMemberWithdrawalPushEventData) GetTransactionFee() int64 {
	if x != nil {
		return x.TransactionFee
	}
	return 0
}

func (x *EVMemberWithdrawalPushEventData) GetIsReviewEvent() bool {
	if x != nil {
		return x.IsReviewEvent
	}
	return false
}

func (x *EVMemberWithdrawalPushEventData) GetReviewResult() bool {
	if x != nil {
		return x.ReviewResult
	}
	return false
}

// 账户日志推送事件
type EVAccountLogPushEventData struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// 账户类型
	Account int32 `protobuf:"varint,1,opt,name=account,proto3" json:"account"`
	// 是否为更新日志事件
	IsUpdateEvent bool `protobuf:"varint,2,opt,name=isUpdateEvent,proto3" json:"isUpdateEvent"`
	// 会员编号
	UserId int64 `protobuf:"varint,3,opt,name=userId,proto3" json:"userId"`
	// 编号
	LogId int64 `protobuf:"varint,4,opt,name=logId,proto3" json:"logId"`
	// 业务类型
	LogKind int32 `protobuf:"varint,5,opt,name=logKind,proto3" json:"logKind"`
	// 标题
	Subject string `protobuf:"bytes,6,opt,name=subject,proto3" json:"subject"`
	// 外部订单号
	OuterNo string `protobuf:"bytes,7,opt,name=outerNo,proto3" json:"outerNo"`
	// 变动金额
	ChangeValue int64 `protobuf:"varint,8,opt,name=changeValue,proto3" json:"changeValue"`
	// 余额
	Balance int64 `protobuf:"varint,9,opt,name=balance,proto3" json:"balance"`
	// 交易手续费
	TransactionFee int64 `protobuf:"varint,10,opt,name=transactionFee,proto3" json:"transactionFee"`
	// 审核状态
	ReviewStatus int32 `protobuf:"varint,11,opt,name=reviewStatus,proto3" json:"reviewStatus"`
	// 创建时间
	CreateTime    int64 `protobuf:"varint,12,opt,name=createTime,proto3" json:"createTime"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *EVAccountLogPushEventData) Reset() {
	*x = EVAccountLogPushEventData{}
	mi := &file_message_events_dto_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EVAccountLogPushEventData) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EVAccountLogPushEventData) ProtoMessage() {}

func (x *EVAccountLogPushEventData) ProtoReflect() protoreflect.Message {
	mi := &file_message_events_dto_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EVAccountLogPushEventData.ProtoReflect.Descriptor instead.
func (*EVAccountLogPushEventData) Descriptor() ([]byte, []int) {
	return file_message_events_dto_proto_rawDescGZIP(), []int{9}
}

func (x *EVAccountLogPushEventData) GetAccount() int32 {
	if x != nil {
		return x.Account
	}
	return 0
}

func (x *EVAccountLogPushEventData) GetIsUpdateEvent() bool {
	if x != nil {
		return x.IsUpdateEvent
	}
	return false
}

func (x *EVAccountLogPushEventData) GetUserId() int64 {
	if x != nil {
		return x.UserId
	}
	return 0
}

func (x *EVAccountLogPushEventData) GetLogId() int64 {
	if x != nil {
		return x.LogId
	}
	return 0
}

func (x *EVAccountLogPushEventData) GetLogKind() int32 {
	if x != nil {
		return x.LogKind
	}
	return 0
}

func (x *EVAccountLogPushEventData) GetSubject() string {
	if x != nil {
		return x.Subject
	}
	return ""
}

func (x *EVAccountLogPushEventData) GetOuterNo() string {
	if x != nil {
		return x.OuterNo
	}
	return ""
}

func (x *EVAccountLogPushEventData) GetChangeValue() int64 {
	if x != nil {
		return x.ChangeValue
	}
	return 0
}

func (x *EVAccountLogPushEventData) GetBalance() int64 {
	if x != nil {
		return x.Balance
	}
	return 0
}

func (x *EVAccountLogPushEventData) GetTransactionFee() int64 {
	if x != nil {
		return x.TransactionFee
	}
	return 0
}

func (x *EVAccountLogPushEventData) GetReviewStatus() int32 {
	if x != nil {
		return x.ReviewStatus
	}
	return 0
}

func (x *EVAccountLogPushEventData) GetCreateTime() int64 {
	if x != nil {
		return x.CreateTime
	}
	return 0
}

var File_message_events_dto_proto protoreflect.FileDescriptor

const file_message_events_dto_proto_rawDesc = "" +
	"\n" +
	"\x18message/events_dto.proto\"_\n" +
	"\x15EVItemAffiliateConfig\x12\x12\n" +
	"\x04rate\x18\x02 \x01(\x01R\x04rate\x12\x16\n" +
	"\x06amount\x18\x03 \x01(\x03R\x06amount\x12\x1a\n" +
	"\bisByRate\x18\x04 \x01(\bR\bisByRate\"\xca\x01\n" +
	"\x14EVOrderAffiliateItem\x12\x16\n" +
	"\x06itemId\x18\x01 \x01(\x03R\x06itemId\x12\x14\n" +
	"\x05skuId\x18\x02 \x01(\x03R\x05skuId\x12\x1a\n" +
	"\bquantity\x18\x03 \x01(\x05R\bquantity\x12\x16\n" +
	"\x06amount\x18\x04 \x01(\x03R\x06amount\x12 \n" +
	"\vfinalAmount\x18\x05 \x01(\x03R\vfinalAmount\x12.\n" +
	"\x06Params\x18\x06 \x03(\v2\x16.EVItemAffiliateConfigR\x06Params\"\xcc\x01\n" +
	"\x19EVOrderAffiliateEventData\x12\x18\n" +
	"\aorderNo\x18\x01 \x01(\tR\aorderNo\x12\x1a\n" +
	"\bsubOrder\x18\x02 \x01(\bR\bsubOrder\x12\x18\n" +
	"\abuyerId\x18\x03 \x01(\x03R\abuyerId\x12 \n" +
	"\vorderAmount\x18\x04 \x01(\x03R\vorderAmount\x12=\n" +
	"\x0eaffiliateItems\x18\x05 \x03(\v2\x15.EVOrderAffiliateItemR\x0eaffiliateItems\"\xdc\x01\n" +
	"\x12EVSendSmsEventData\x12\x1a\n" +
	"\bprovider\x18\x01 \x01(\x05R\bprovider\x12\x14\n" +
	"\x05phone\x18\x02 \x01(\tR\x05phone\x12\x1a\n" +
	"\btemplate\x18\x03 \x01(\tR\btemplate\x12\"\n" +
	"\ftemplateCode\x18\x04 \x01(\tR\ftemplateCode\x12\"\n" +
	"\fspTemplateId\x18\x05 \x01(\tR\fspTemplateId\x12\x1c\n" +
	"\tsignature\x18\x06 \x01(\tR\tsignature\x12\x12\n" +
	"\x04data\x18\a \x03(\tR\x04data\"\xb5\x03\n" +
	"\x15EVMemberPushEventData\x12\x1a\n" +
	"\bmemberId\x18\x01 \x01(\x03R\bmemberId\x12 \n" +
	"\visNewMember\x18\x02 \x01(\bR\visNewMember\x12\x1a\n" +
	"\buserCode\x18\x03 \x01(\tR\buserCode\x12\x1a\n" +
	"\buserFlag\x18\x04 \x01(\x11R\buserFlag\x12\x12\n" +
	"\x04role\x18\x05 \x01(\x11R\x04role\x12\x1a\n" +
	"\busername\x18\x06 \x01(\tR\busername\x12 \n" +
	"\vcountryCode\x18\a \x01(\tR\vcountryCode\x12\x14\n" +
	"\x05level\x18\b \x01(\x11R\x05level\x12\x1a\n" +
	"\bnickname\x18\t \x01(\tR\bnickname\x12\"\n" +
	"\fprofilePhoto\x18\n" +
	" \x01(\tR\fprofilePhoto\x12\x14\n" +
	"\x05phone\x18\f \x01(\tR\x05phone\x12\x14\n" +
	"\x05email\x18\r \x01(\tR\x05email\x12\x18\n" +
	"\aregFrom\x18\x0e \x01(\tR\aregFrom\x12\x1a\n" +
	"\brealName\x18\x0f \x01(\tR\brealName\x12\x1c\n" +
	"\tinviterId\x18\x10 \x01(\x03R\tinviterId\"[\n" +
	"\x17EVRegistryPushEventData\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value\x12\x18\n" +
	"\aisUsers\x18\x03 \x01(\bR\aisUsers\"\xef\x01\n" +
	"\x17EVSubOrderPushEventData\x12\x18\n" +
	"\aorderNo\x18\x01 \x01(\tR\aorderNo\x12 \n" +
	"\vorderAmount\x18\x02 \x01(\x03R\vorderAmount\x12$\n" +
	"\rconsigneeName\x18\x03 \x01(\tR\rconsigneeName\x12&\n" +
	"\x0econsigneePhone\x18\x04 \x01(\tR\x0econsigneePhone\x12*\n" +
	"\x10consigneeAddress\x18\x05 \x01(\tR\x10consigneeAddress\x12\x1e\n" +
	"\n" +
	"orderState\x18\x06 \x01(\x05R\n" +
	"orderState\"\xdc\x02\n" +
	"\x18EVMemberAccountEventData\x12\x1a\n" +
	"\bmemberId\x18\x01 \x01(\x03R\bmemberId\x12\x1a\n" +
	"\bintegral\x18\x02 \x01(\x03R\bintegral\x12\x18\n" +
	"\abalance\x18\x03 \x01(\x03R\abalance\x12\x1e\n" +
	"\n" +
	"walletCode\x18\x04 \x01(\tR\n" +
	"walletCode\x12$\n" +
	"\rwalletBalance\x18\x05 \x01(\x03R\rwalletBalance\x12 \n" +
	"\vflowBalance\x18\x06 \x01(\x03R\vflowBalance\x12 \n" +
	"\vgrowBalance\x18\a \x01(\x03R\vgrowBalance\x12\"\n" +
	"\ftotalExpense\x18\b \x01(\x03R\ftotalExpense\x12 \n" +
	"\vtotalCharge\x18\t \x01(\x03R\vtotalCharge\x12\x1e\n" +
	"\n" +
	"updateTime\x18\x14 \x01(\x03R\n" +
	"updateTime\"\xe5\x01\n" +
	"\x1fEVMemberWithdrawalPushEventData\x12\x1a\n" +
	"\bmemberId\x18\x01 \x01(\x03R\bmemberId\x12\x1c\n" +
	"\trequestId\x18\x02 \x01(\x03R\trequestId\x12\x16\n" +
	"\x06amount\x18\x03 \x01(\x03R\x06amount\x12&\n" +
	"\x0etransactionFee\x18\x04 \x01(\x03R\x0etransactionFee\x12$\n" +
	"\risReviewEvent\x18\x05 \x01(\bR\risReviewEvent\x12\"\n" +
	"\freviewResult\x18\x06 \x01(\bR\freviewResult\"\xff\x02\n" +
	"\x19EVAccountLogPushEventData\x12\x18\n" +
	"\aaccount\x18\x01 \x01(\x05R\aaccount\x12$\n" +
	"\risUpdateEvent\x18\x02 \x01(\bR\risUpdateEvent\x12\x16\n" +
	"\x06userId\x18\x03 \x01(\x03R\x06userId\x12\x14\n" +
	"\x05logId\x18\x04 \x01(\x03R\x05logId\x12\x18\n" +
	"\alogKind\x18\x05 \x01(\x05R\alogKind\x12\x18\n" +
	"\asubject\x18\x06 \x01(\tR\asubject\x12\x18\n" +
	"\aouterNo\x18\a \x01(\tR\aouterNo\x12 \n" +
	"\vchangeValue\x18\b \x01(\x03R\vchangeValue\x12\x18\n" +
	"\abalance\x18\t \x01(\x03R\abalance\x12&\n" +
	"\x0etransactionFee\x18\n" +
	" \x01(\x03R\x0etransactionFee\x12\"\n" +
	"\freviewStatus\x18\v \x01(\x05R\freviewStatus\x12\x1e\n" +
	"\n" +
	"createTime\x18\f \x01(\x03R\n" +
	"createTimeB\x1f\n" +
	"\x13com.github.go2o.rpcZ\b./;protob\x06proto3"

var (
	file_message_events_dto_proto_rawDescOnce sync.Once
	file_message_events_dto_proto_rawDescData []byte
)

func file_message_events_dto_proto_rawDescGZIP() []byte {
	file_message_events_dto_proto_rawDescOnce.Do(func() {
		file_message_events_dto_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_message_events_dto_proto_rawDesc), len(file_message_events_dto_proto_rawDesc)))
	})
	return file_message_events_dto_proto_rawDescData
}

var file_message_events_dto_proto_msgTypes = make([]protoimpl.MessageInfo, 10)
var file_message_events_dto_proto_goTypes = []any{
	(*EVItemAffiliateConfig)(nil),           // 0: EVItemAffiliateConfig
	(*EVOrderAffiliateItem)(nil),            // 1: EVOrderAffiliateItem
	(*EVOrderAffiliateEventData)(nil),       // 2: EVOrderAffiliateEventData
	(*EVSendSmsEventData)(nil),              // 3: EVSendSmsEventData
	(*EVMemberPushEventData)(nil),           // 4: EVMemberPushEventData
	(*EVRegistryPushEventData)(nil),         // 5: EVRegistryPushEventData
	(*EVSubOrderPushEventData)(nil),         // 6: EVSubOrderPushEventData
	(*EVMemberAccountEventData)(nil),        // 7: EVMemberAccountEventData
	(*EVMemberWithdrawalPushEventData)(nil), // 8: EVMemberWithdrawalPushEventData
	(*EVAccountLogPushEventData)(nil),       // 9: EVAccountLogPushEventData
}
var file_message_events_dto_proto_depIdxs = []int32{
	0, // 0: EVOrderAffiliateItem.Params:type_name -> EVItemAffiliateConfig
	1, // 1: EVOrderAffiliateEventData.affiliateItems:type_name -> EVOrderAffiliateItem
	2, // [2:2] is the sub-list for method output_type
	2, // [2:2] is the sub-list for method input_type
	2, // [2:2] is the sub-list for extension type_name
	2, // [2:2] is the sub-list for extension extendee
	0, // [0:2] is the sub-list for field type_name
}

func init() { file_message_events_dto_proto_init() }
func file_message_events_dto_proto_init() {
	if File_message_events_dto_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_message_events_dto_proto_rawDesc), len(file_message_events_dto_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   10,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_message_events_dto_proto_goTypes,
		DependencyIndexes: file_message_events_dto_proto_depIdxs,
		MessageInfos:      file_message_events_dto_proto_msgTypes,
	}.Build()
	File_message_events_dto_proto = out.File
	file_message_events_dto_proto_goTypes = nil
	file_message_events_dto_proto_depIdxs = nil
}
