...
func superviseOrder(ss []Service) {
	...
	for {
		...
		if err == nil {
			...
			provide.GetApp().Log().Println("[ Daemon][ OrderQueue][ Error]:",
...
func superviseMemberUpdate(ss []Service) {
	...
	for {
		...
		if err == nil {
			...
			provide.GetApp().Log().Println("[ Daemon][ MemberQueue][ Error]:",
				err.Error(), "; retry after 10 seconds.")
			time.Sleep(time.Second * 10)
		}
	}
...
func supervisePaymentOrderFinish(ss []Service) {
	...
	for {
		...
		if err == nil {
			...
			provide.GetApp().Log().Println("[ Daemon][ PaymentOrderQueue][ Error]:",
				err.Error(), "; retry after 10 seconds.")
			time.Sleep(time.Second * 10)
		}
	}
