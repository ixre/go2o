...
func generateMchDayChart(start, end int64) {
	...
	// 清理数据
	provide.GetApp().Db().ExecNonQuery(`DELETE FROM mch_day_chart WHERE date_str= $1`, dateStr)
	// 开始统计数据
	for {
		mchList = []int64{}
		provide.GetApp().Db().Query("SELECT id FROM mch_merchant LIMIT $2 OFFSET $1", func(rs *sql.Rows) {
			for rs.Next() {
				rs.Scan(&tmp)
