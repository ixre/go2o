...
func (this *advertisementRep) GetAdPositionByKey(key string) *ad.AdPosition {
    e := ad.AdPosition{}
    if err := this.GetOrm().GetBy(&e, "ad_position.key=?", key); err != nil {
        handleError(err)
        return nil
    }
    return &e
...
// 根据名称获取广告编号
func (this *advertisementRep) GetIdByName(userId int, name string) int {
    var id int
    this.Connector.ExecScalar("SELECT id FROM ad_list WHERE user_id=? AND name=?",
        &id, userId, name)
    return id
}
...
func (this *advertisementRep) SaveAdValue(v *ad.Ad) (int, error) {
    ...
    if v.Id > 0 {
        ...
        _, _, err = orm.Save(nil, v)
        this.Connector.ExecScalar("SELECT MAX(id) FROM ad_list WHERE user_id=?",
            &v.Id, v.UserId)
    }
    this.Mutex.Unlock()
    return v.Id, err
...
func (this *advertisementRep) GetAdByKey(userId int, key string) *ad.Ad {
    ...
    if err := this.Connector.GetOrm().GetByQuery(&e, sql, userId, key); err == nil {
        return &e
    }
    return nil
}
...
// 删除广告
func (this *advertisementRep) DelAd(userId, advertisementId int) error {
    _, err := this.Connector.GetOrm().Delete(ad.Ad{}, "user_id=? AND id=?", userId, advertisementId)
    return err
}
