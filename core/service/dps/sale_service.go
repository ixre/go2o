...
func (s *saleService) GetPagedOnShelvesGoods(categoryId, start, end int,
	sortBy string) (total int, list []*valueobject.Goods) {
	if categoryId > 0 {
		cate := s._cateRep.GetGlobManager().GetCategory(categoryId)
		var ids []int = cate.GetChildes()
		ids = append(ids, categoryId)
		total, list = s._goodsRep.GetPagedOnShelvesGoods(0, ids, start, end, "", sortBy)
	} else {
		total = -1
		total, list = s._goodsRep.GetPagedOnShelvesGoods(0, []int{}, start, end, "", sortBy)
	}
	for _, v := range list {
		v.Image = format.GetGoodsImageUrl(v.Image)
	}
