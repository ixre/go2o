...
// 保存资料
func (ms *memberService) SaveProfile(memberId int32, v *member.Profile) error {
...
// 取消收藏
func (ms *memberService) Cancel(memberId, favType, referId int32) error {
	return ms._rep.CreateMemberById(memberId).
...
// 注册会员
func (ms *memberService) RegisterMember(mchId int32, v *member.Member,
...
//保存配送地址
func (ms *memberService) SaveAddress(memberId int32, e *member.DeliverAddress) (int, error) {
	m := ms._rep.CreateMember(&member.Member{Id: memberId})
...
//删除配送地址
func (ms *memberService) DeleteAddress(memberId int32, deliverId int) error {
	m := ms._rep.CreateMember(&member.Member{Id: memberId})
...
// 会员推广排名
func (ms *memberService) GetMemberInviRank(mchId int32, allTeam bool, levelComp string, level int,
