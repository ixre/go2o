...
func Usage() {
  ...
  fmt.Fprintln(os.Stderr, "  SSmsApi GetSmsApi(string provider)")
  fmt.Fprintln(os.Stderr, "  Result SaveSmsApi(string provider, SSmsApi api)")
  fmt.Fprintln(os.Stderr, "  Result SaveBoardHook(string hookURL, string token)")
...
func main() {
  ...
  case "GetSmsApi":
    if flag.NArg() - 1 != 1 {
      fmt.Fprintln(os.Stderr, "GetSmsApi requires 1 args")
      flag.Usage()
    }
    argvalue0 := flag.Arg(1)
    value0 := argvalue0
    fmt.Print(client.GetSmsApi(context.Background(), value0))
    fmt.Print("\n")
    break
  case "SaveSmsApi":
    if flag.NArg() - 1 != 2 {
    ...
    argvalue1 := foundation_service.NewSSmsApi()
    err87 := argvalue1.Read(jsProt86)
    if err87 != nil {
      Usage()
      return
    }
    value1 := foundation_service.SSmsApi(argvalue1)
    fmt.Print(client.SaveSmsApi(context.Background(), value0, value1))
    fmt.Print("\n")
    break
  case "SaveBoardHook":
    if flag.NArg() - 1 != 2 {
      fmt.Fprintln(os.Stderr, "SaveBoardHook requires 2 args")
      flag.Usage()
    }
    argvalue0 := flag.Arg(1)
    value0 := argvalue0
    argvalue1 := flag.Arg(2)
    value1 := argvalue1
    fmt.Print(client.SaveBoardHook(context.Background(), value0, value1))
    fmt.Print("\n")
    break
  case "ResourceUrl":
    if flag.NArg() - 1 != 1 {
