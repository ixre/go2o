...
type ContentService interface { //内容服务
	...
	//  - Begin_
	//  - Size
	QueryPagingArticles(ctx context.Context, cat string, begin_ int32, size int32) (r *ttype.SPagingResult_, err error)
	// 获取置顶的文章
	//
	// Parameters:
	//  - Cat
...
//  - Begin_
//  - Size
func (p *ContentServiceClient) QueryPagingArticles(ctx context.Context, cat string, begin_ int32, size int32) (r *ttype.SPagingResult_, err error) {
	var _args0 ContentServiceQueryPagingArticlesArgs
	_args0.Cat = cat
	_args0.Begin_ = begin_
	_args0.Size = size
...
func (p *contentServiceProcessorQueryPagingArticles) Process(ctx context.Context, seqId int32, iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
	...
	var err2 error
	if retval, err2 = p.handler.QueryPagingArticles(ctx, args.Cat, args.Begin_, args.Size); err2 != nil {
...
type ContentServiceQueryPagingArticlesArgs struct {
	...
	Begin_ int32  `thrift:"begin_,2" db:"begin_" json:"begin_"`
...
func (p *ContentServiceQueryPagingArticlesArgs) GetBegin_() int32 {
	return p.Begin_
}
...
func (p *ContentServiceQueryPagingArticlesArgs) writeField2(oprot thrift.TProtocol) (err error) {
	if err := oprot.WriteFieldBegin("begin_", thrift.I32, 2); err != nil {
		return thrift.PrependError(fmt.Sprintf("%T write field begin error 2:begin_: ", p), err)
	}
	if err := oprot.WriteI32(int32(p.Begin_)); err != nil {
		return thrift.PrependError(fmt.Sprintf("%T.begin_ (2) field write error: ", p), err)
	}
	if err := oprot.WriteFieldEnd(); err != nil {
		return thrift.PrependError(fmt.Sprintf("%T write field end error 2:begin_: ", p), err)
	}
	return err
}
