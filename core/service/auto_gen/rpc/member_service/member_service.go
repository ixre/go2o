...
type SMemberLevelInfo struct {
	...
	RequireExp    int32  `thrift:"RequireExp,7" db:"RequireExp" json:"RequireExp"`
}
...
func (p *SMemberLevelInfo) writeField7(oprot thrift.TProtocol) (err error) {
	if err := oprot.WriteFieldBegin("RequireExp", thrift.I32, 7); err != nil {
		return thrift.PrependError(fmt.Sprintf("%T write field begin error 7:RequireExp: ", p), err)
	}
	if err := oprot.WriteI32(int32(p.RequireExp)); err != nil {
		return thrift.PrependError(fmt.Sprintf("%T.RequireExp (7) field write error: ", p), err)
	}
	if err := oprot.WriteFieldEnd(); err != nil {
		return thrift.PrependError(fmt.Sprintf("%T write field end error 7:RequireExp: ", p), err)
	}
	return err
}
