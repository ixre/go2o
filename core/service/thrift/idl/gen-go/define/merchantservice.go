...
func NewMerchantServiceProcessor(handler MerchantService) *MerchantServiceProcessor {
	...
	self190 := &MerchantServiceProcessor{handler: handler, processorMap: make(map[string]thrift.TProcessorFunction)}
	self190.processorMap["Complex"] = &merchantServiceProcessorComplex{handler: handler}
	self190.processorMap["CheckLogin"] = &merchantServiceProcessorCheckLogin{handler: handler}
	self190.processorMap["Stat"] = &merchantServiceProcessorStat{handler: handler}
	self190.processorMap["SyncWholesaleItem"] = &merchantServiceProcessorSyncWholesaleItem{handler: handler}
	return self190
}
