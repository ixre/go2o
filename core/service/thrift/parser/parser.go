...
func PaymentOrder(src *define.SPaymentOrder) *payment.PaymentOrder {
	return &payment.PaymentOrder{
		...
		SellerId:  int(src.VendorId),
		//Type:             src.Type,
		//OrderId:          src.OrderId,
		Subject:     src.Subject,
		BuyerId:     src.BuyUser,
		PayUid:      src.PaymentUser,
		...
		PayFlag:  int(src.PaymentOptFlag),
		//PaymentSign:      src.PaymentSign,
		OutTradeNo: src.OuterNo,
...
func PaymentOrderDto(src *payment.PaymentOrder) *define.SPaymentOrder {
	return &define.SPaymentOrder{
		...
		TradeNo:   src.TradeNo,
		TradeType: src.TradeType,
		VendorId:  int32(src.SellerId),
		//Type:             src.Type,
		//OrderId:          src.OrderId,
		Subject:     src.Subject,
		BuyUser:     src.BuyerId,
		PaymentUser: src.PayUid,
		TotalAmount: round(float32(src.TotalAmount), 2),
		...
		FinalFee:       round(float32(src.FinalFee), 2),
		PaymentOptFlag: int32(src.PayFlag),
		//PaymentSign:      src.PaymentSign,
		OuterNo:    src.OutTradeNo,
...
func ItemDto(src *item.GoodsItem) *define.OldItem {
	it := &define.OldItem{
		ItemId:       src.ID,
...
func Item(src *define.OldItem) *item.GoodsItem {
	it := &item.GoodsItem{
		ID:           src.ItemId,
		ProductId:    src.ProductId,
		PromFlag:     src.PromFlag,
