...
var (
	std ILogger = NewLogger(os.Stderr, "", LOpen|LStdFlags|LSource)
)
...
func (t *simpleLogger) formatHeader(b *[]byte) {
	if t.flag&LStdFlags != 0 {
		now := time.Now()
		*b = append(*b, now.Format("2006-01-02 15:03:04 ** ")...)
		if len(t.prefix) != 0 {
			*b = append(*b, t.prefix...)
		}
	}
...
func (t *simpleLogger) output(b *[]byte, newLine bool) {
	t.mux.Lock()
	defer t.mux.Unlock()
	if t.flag&LSource != 0 {
...
func (t *simpleLogger) Printf(s string, v ...interface{}) {
	if t.opened {
		...
		t.output(&t.buf, false)
	}
}

func (t *simpleLogger) PrintErr(e error) {
	if t.opened && e != nil {
		t.Println("[ Error] -", e.Error())
	}
}
